...
=======
	...
	const handleClickOpen = () => {
		...
		window.sessionStorage.setItem(
			"teamMembers",
			JSON.stringify({ members: [user.user_id] })
		);
	};
	const handleClose = () => {
		setOpen(false);
	...
	const addToLadder = async (e) => {
		...
		if (exists) {
			console.log(
				`exists is ${exists} & team member id is ${
					JSON.parse(window.sessionStorage.getItem("team_member")).user_id
				}`
			);

			let ssAddMember = JSON.parse(
				window.sessionStorage.getItem("team_member")
			).user_id;
...
=======
			...
			let newSSMembers = JSON.parse(
				window.sessionStorage.getItem("teamMembers")
			);
